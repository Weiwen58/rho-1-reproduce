...
class OpenWebMath(DataModule):
    ...
    data_path: Union[str, Path] = Path("data/OpenWebMath")
    ...
    def prepare_data(self) -> None:
        ...
        if str(self.data_path).startswith("s3://"):
            print(f"The OpenWebMath data path points to an S3 location: {self.data_path}. Skipping preprocessing.")
            return

        if Path(self.data_path_train).is_dir() and Path(self.data_path_val).is_dir():
            print(f"Found OpenWebMath train and val dir: {self.data_path}. Skipping preprocessing.")
            return

        dataset = load_dataset("open-web-math/open-web-math", num_proc=(os.cpu_count() // 2), trust_remote_code=True)
