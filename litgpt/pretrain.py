...
def main(
    ...
    with fabric.init_module(empty_init=True):
        model = GPT(config)  # primary model for training
        reference_model = GPT(config)  # reference model for loss computation
    ...
    # Set up the reference model (no compilation or optimizer needed)
    ...
    fit(fabric, devices, state, train_dataloader, val_dataloader, out_dir, tokenizer_dir, train, eval, reference_model)
...
def compute_excess_loss(model, reference_model, input_ids, targets):
    """Compute excess loss (L_Δ) between training model and reference model."""
    # Get per-token loss for the training model
    logits = model(input_ids)
    training_loss = torch.nn.functional.cross_entropy(
        logits.permute(0, 2, 1), targets, reduction="none"
    )

    # Get per-token loss for the reference model
    with torch.no_grad():
        r_logits = reference_model(input_ids)
        reference_loss = torch.nn.functional.cross_entropy(
            r_logits.permute(0, 2, 1), targets, reduction="none"
        )

    # Compute excess loss
    excess_loss = training_loss - reference_loss
    return excess_loss, logits
...
def selective_training_step(
    ...
    excess_loss, logits = compute_excess_loss(model, reference_model, input_ids, targets)

    B, T = input_ids.size()
    
    # Select top k% tokens based on excess loss
    k = max(1, int(T * selection_ratio))  # Ensure at least one token is selected
    _, selected_indices = torch.topk(excess_loss, k, dim=1)  # Shape: (B, k)

    # Create mask for selected tokens
    selection_mask = torch.zeros_like(targets, dtype=torch.bool)  # Shape: (B, T)
    batch_indices = torch.arange(B, device=input_ids.device).unsqueeze(1)  # Shape: (B, 1)
    ...
    return torch.nn.functional.cross_entropy(logits, new_targets, ignore_index=ignore_index)